...
\begin{align*}
  \mathbb{P}\left[U_i \le t\right] &= \mathbb{P}\left[F(X_i) \le t\right] &\quad&\text{(definition of $U_i$)} \\
                   ...
                   &= t \\
  \therefore F_U(t) &= t \\
  \implies f_U(t) &= \mathcal{U}\left(\left[ 0, 1 \right]\right) \tag*{$\unicode{x25A1}$}
\end{align*}
$$
...
{3}
    F(t) &= \mathbb{P}\left[X \le t\right] \quad \forall t \in \mathbb{R}\\
         &= \mathbb{E}\left[\indicator\!\left\{X \le t\right\}\right].
\end{align*}
$$

By the Law of Large Numbers, the expectation can be approximated by the sample
average, so we can define the \emph{empirical cdf} as

$$
\begin{align*}
    F_n(t) &= \frac{1}{n}\sum_{i=1}^{n} \indicator\!\left\{X_i \le t\right\} \tag{\theequation}\label{eq:F_n}
\end{align*}
$$
...
$$
\begin{align*}
    G_m(t) &= \frac{1}{m}\sum_{j=1}^{m} \indicator\!\left\{Y_j \le t\right\}. \tag{\theequation}\label{eq:G_m}
  \end{align*}
$$

$$
\therefore T_{n,m} = \sup_{t \in \mathbb{R}} \left| \frac{1}{n}\sum_{i=1}^{n} \indicator\!\left\{X_i \le t\right\} - \frac{1}{m}\sum_{j=1}^{m} \indicator\!\left\{Y_j \le t\right\} \right|.
$$
...
\begin{split}
      ...
      &\max_{j=0,\dots,m} \left| \frac{i}{n} - \frac{j}{m} \right| 
        \indicator\!\left\{Y^{(j)} \le X^{(i)} < Y^{(j+1)}\right\}, \\ 
      &\max_{k=j+1, \dots, m} \left| \frac{i}{n} - \frac{k}{m} \right| 
        \indicator\!\left\{Y^{(k)} \le X^{(i+1)}\right\} \Bigg]
    \end{split}
$$
...
$$
T_{n,m} = \sup_{0 \le x \le 1} \left| \frac{1}{n}\sum_{i=1}^{n} \indicator\!\left\{U_i \le x\right\}
- \frac{1}{m}\sum_{j=1}^{m} \indicator\!\left\{V_j \le x\right\} \right|.
$$
...
$$
\label{eq:Tnm_supt}
    T_{n,m} = \sup_{t \in \mathbb{R}} \left| \frac{1}{n}\sum_{i=1}^{n} \indicator\!\left\{X_i \le t\right\} - \frac{1}{m}\sum_{j=1}^{m} \indicator\!\left\{Y_j \le t\right\} \right|.
$$
...
\begin{align*}
  q_\alpha &= F^{-1}(1-\alpha) \\
           ...
           \implies q_\alpha \approx \hat{q}_\alpha &= \min_i \left\{
               T_{n,m}^{(i)} \colon \tfrac{i}{M} \ge 1 - \alpha \right\}\end{align*}
$$

where $M \in \mathbb{N}$ is large, and $T_{n,m}^{(i)}$ is the
$i^\text{th}$ value in a sorted sample of $M$ test statistics. Thus,
$q_\alpha$ can be approximated by choosing $i = \lceil M(1 - \alpha) \rceil$.
...
$j \gets \lceil M(1 - \alpha) \rceil$ $T_{vs}^{(j)}$.
...
$$
\delta_\alpha = \indicator\!\left\{T_{n,m} > \hat{q}_\alpha^{(n, M)}\right\}
$$
...
\begin{align*}
  ...
  &\approx \frac{\{j = 1, \dots, M \colon T_{n,m}^{(j)} \ge T_{n,m}\}}{M}\end{align*}
$$
